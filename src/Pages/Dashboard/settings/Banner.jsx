  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <label
          htmlFor="img"
          className="bg-primary text-white flex items-center justify-center h-10 px-4 cursor-pointer rounded-md"
        >
          Add Banner
        </label>
        <input
      ...
      <div className="flex items-center gap-10">
        {banners?.map((url, index) => {
          return (
            <div
              key={index}
              className="w-full h-[200px] border relative rounded-lg p-2"
            >
              <img
                style={{ width: "100%", height: "100%", objectFit: "contain" }}
                src={`${imageUrl}${url?.banner}`}
                alt=""
              />
              <div className="absolute top-[10px] right-[10px] flex items-center gap-3">
                ...
                <Checkbox
                  onChange={() => handleStatus(url._id)}
                  checked={url?.status}
                >
                  {url?.status ? "Show" : "Hide"}
                </Checkbox>
              </div>
            </div>
          );
